...
# Constants
LUNCH_WEBSOCKET_SERVICE="lunch-websocket"
...
check_server_status() {
   echo "Server Status:"
   systemctl is-active $LUNCH_WEBSOCKET_SERVICE
}
...
restart_websocket_service() {
   echo "Restarting $LUNCH_WEBSOCKET_SERVICE service..."
   systemctl restart $LUNCH_WEBSOCKET_SERVICE
   systemctl status $LUNCH_WEBSOCKET_SERVICE --no-pager  # Optionally, show status after restarting
}

# Function to uninstall proxy script
uninstall_proxy_script() {
   echo "Stopping $LUNCH_WEBSOCKET_SERVICE service..."
   systemctl stop $LUNCH_WEBSOCKET_SERVICE

   echo "Disabling $LUNCH_WEBSOCKET_SERVICE service..."
   systemctl disable $LUNCH_WEBSOCKET_SERVICE

   echo "Removing Python proxy files..."
   rm -rf "/etc/lunchkit/lunch_websocket"
   rm /usr/local/bin/lunchws

   echo "Removing systemd service file..."
   rm -f "/etc/systemd/system/$LUNCH_WEBSOCKET_SERVICE.service"
...
server_information() {
   ...
   if systemctl is-active --quiet $LUNCH_WEBSOCKET_SERVICE; then
